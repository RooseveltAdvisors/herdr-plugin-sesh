...
herdr plugin action invoke RooseveltAdvisors.herdr-sesh.last
```
...
=== "Installed plugin"
    ...
    ```bash
    sesh_root="$(herdr plugin list --plugin RooseveltAdvisors.herdr-sesh --json | jq -r '.result.plugins[0].plugin_root')"
    sesh_bin="$sesh_root/bin/herdr-sesh"
    export HERDR_PLUGIN_CONFIG_DIR="$(herdr plugin config-dir RooseveltAdvisors.herdr-sesh)"
    ```
...
!!! note "Use the Herdr action for previous-workspace history"
    ...
    has history. Use `herdr plugin action invoke RooseveltAdvisors.herdr-sesh.last` to inherit
