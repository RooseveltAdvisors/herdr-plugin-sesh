...
!!! info "Protocol 20 replay is reconciled against current session state"
    ...
    skip that replay step; see the [subscriber implementation](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/herdr/events.go)
    and [protocol regression tests](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/herdr/events_test.go).
...
```text
${HERDR_PLUGIN_STATE_DIR}/sessions/<hex-HERDR_SESSION>/history/<sha256-clean-socket-path>/history.json
```

The `sessions/<hex-HERDR_SESSION>` segment is this fork's addition and is omitted
for the default session, so socket scoping still works on its own when
`HERDR_SOCKET_PATH` is unavailable. Every entry point resolves the directory
through `app.historyStateDir`; a command that resolves it any other way reads a
...
In this fork the toggle destination does not come from the list. `Last` and
`Previous` are thin wrappers over the two-slot `FocusMRU` pair in
`internal/state/mru.go`: `Record` is `ObserveWorkspaceFocus`, `RecordSwitch` is
`ConsumeWorkspaceToggle`, and `RemoveWorkspace` is `ClearWorkspace`. The list
survives only as picker recency ordering. Tab/agent focus has no Herdr event
subscription, so `tab.focused` and `tab.closed` stay wired to the `hook` command
and update the agent half of the same pair under the same `history.lock`.
...
[Watcher orchestration](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/app/app.go){ .md-button }
[Event subscriber](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/herdr/events.go){ .md-button }
[History state](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/state/history.go){ .md-button }

[^watcher-source]: See the lifecycle declarations in
    [`herdr-plugin.toml`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/herdr-plugin.toml)
    and watcher election in
    [`internal/app/app.go`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/app/app.go).
[^snapshot-source]: See `loadSessionSnapshot` and the buffered decoder in
    [`internal/herdr/events.go`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/herdr/events.go).
...
[^persistence-source]: The lock and state rules are in
    [`internal/state/history.go`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/state/history.go);
    atomic replacement is in
    [`internal/state/json.go`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/state/json.go).
[^record-switch-source]: See `RecordSwitch` in
    [`internal/state/history.go`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/state/history.go)
    and its focused tests in
    [`internal/state/history_test.go`](https://github.com/RooseveltAdvisors/herdr-plugin-sesh/blob/main/internal/state/history_test.go).
