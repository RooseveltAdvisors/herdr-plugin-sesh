...
func (a *App) picker(ctx context.Context, args []string) error {
	...
	var ok bool
	focusedWorkspaceID := currentWorkspaceID()
	pickerWorkspaceID := focusedWorkspaceID
	if useFZF {
		...
		pickOpts.RecentWorkspaceIDs = append([]string{focusedWorkspaceID}, history.Workspaces...)
		if cfg.TUI.ShowLastWorkspace {
			pickOpts.HerdrWorkspaces = herdrWorkspaces
			lastWorkspaceID, _, lastWorkspaceErr := lastWorkspace(historyDir, focusedWorkspaceID)
...
func (a *App) last(ctx context.Context, _ []string) error {
	stateDir, err := historyStateDir()
	if err != nil {
		return err
	}
	focusedWorkspaceID := currentWorkspaceID()
	id, ok, err := lastWorkspace(stateDir, focusedWorkspaceID)
	if err != nil {
		return err
	}
	if !ok {
	...
	if err := state.ConsumeWorkspaceToggle(stateDir, focusedWorkspaceID, id); err != nil {
...
// historyStateDir resolves the one state directory every entry point must agree
// on. pluginStateDir scopes by HERDR_SESSION so concurrent Herdr sessions keep
// separate toggle pairs even when the socket path is unavailable; on top of that
// SessionHistoryDir scopes by socket and migrates pre-existing default-session
// history exactly once.
func historyStateDir() (string, error) {
	return state.SessionHistoryDir(pluginStateDir(), os.Getenv("HERDR_SOCKET_PATH"))
}
