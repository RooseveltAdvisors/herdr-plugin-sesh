...
CI runs formatting, vet, tests, build, and CLI smoke checks; mirror those checks before opening a pull request. Documentation changes must also pass `just build-docs`. Run all local checks through `just` recipes - bare `go test`/`go build`/`gofmt` invocations run outside the pinned toolchain (missing golangci-lint and race detection) and do not count as validation.
...
`last` and `last-agent` are strict two-target toggles. Exactly one writer observes each
Herdr event: workspace `focused`/`closed` go through `plugin watch-history` (the socket
subscriber in `internal/herdr/events.go`), and tab `focused`/`closed` go through the
`hook` command; `herdr-plugin.toml` must never wire both to the same event. Both paths
write the same `internal/state` `FocusMRU` pair and share one `history.lock`. Prefer
`HERDR_PLUGIN_EVENT_JSON` over ambient `HERDR_*` for the focused target. Every entry
point must resolve its state directory through `app.historyStateDir` — `HERDR_SESSION`
scoping plus `state.SessionHistoryDir` socket scoping — because a command that resolves
it differently silently reads another session's pair. Do not replace this with an N-item
history cycle. Agent/tab and workspace pairs must stay orthogonal; cross-workspace `last-agent` jumps use `PrepareAgentJump` so they do not rewrite workspace state. Closed or missing targets clear only the saved slot and refuse, rather than falling back to older picker history.
